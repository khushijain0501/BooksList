...
const BookModel = ({book,onClose,caller}) => {
  ...
  return (
    ...
    "fixed bg-black bg-opacity-30 top-0 right-0 bottom-0 left-0 flex justify-center items-center p-10 py-6 z-50 text-left"} onClick={onClose}>
      <div onClick={(event)=>event.stopPropagation()} className='w-[600px] bg-[#48b2b1] max-w-full h-[430px] rounded-xl p-4 m-4 flex flex-col shadow-[3px_4px_8px_0.1px_rgba(0,0,0,0.2)] shadow-[#3d3251] relative'>
      ...
      <div><FaUser className="text-[#3d3251] my-1 mr-2"/></div>
        ...
        <div  className='flex justify-start items-center align-center px-1'>
              {stars.map((_,index)=>{
                return(
                  <div>
                  <FaStar
